...
const defaultItem = items.find((item): boolean => item.value === ELEMENT_PARAGRAPH)!
...
export function TurnIntoDropdownMenu(props: DropdownMenuProps) {
  ...
  if (isCollapsed(editor?.selection)) {
    const entry = findNode<TElement>(editor!, { match: (n): boolean => isBlock(editor, n) })
    if (entry) {
      value = items.find((item): boolean => item.value === entry[0].type)?.value ?? ELEMENT_PARAGRAPH
    }
  }

  const selectedItem = items.find((item): boolean => item.value === value) ?? defaultItem
